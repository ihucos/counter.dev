...
.PHONY: runserver
runserver:
	plash --from alpine:3.11 --apk go -- go run db.go server.go ctx.go handlers.go setup.go
...
deploy:
	plash --from alpine:3.11 --apk go -- go build db.go server.go ctx.go handlers.go setup.go
