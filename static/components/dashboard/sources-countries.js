customElements.define(
    ...
    class extends HTMLElement {
        ...
        drawItemSources(ref, count, totalCount) {
            const item = this.groupItems[ref];
        ...
        groupItem(ref) {
            ...
            for (const groupName in this.GROUP_SOURCES) {
                const groupMeta = this.GROUP_SOURCES[groupName];
        ...
        draw(sources, countries) {
            const groupedSources = { link: null, icon: null };
            const groupItems = {};
            Object.keys(sources).forEach((ref) => {
                const refVisits = sources[ref];
                const groupItem = this.groupItem(ref);
        ...
        getCountryName(countryCode) {
            if (Object.hasOwn(this.COUNTRIES, countryCode)) {
