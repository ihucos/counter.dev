customElements.define(
	...
	class extends HTMLElement {
		...
		drawPlans(userDump) {
			...
			window.setInterval(() => {
				const height = $("#modal-pwyw").height();
		...
		subscriptionSuccess(subscriptionID) {
			...
			$.modal.close();
			notify(
				`You are awesome. If you are not happy with the product or service let us know at any time.`,
			);
		}
		...
		highlightPersonalizedSuggestion(dump) {
				...
				.map(
					...
					(i) =>
						Object.entries(i.visits.all.date)
							...
							.reduce((pv, cv) => pv + cv, 0) / 7, // sum them and divide by 7
				)
				.reduce((pv, cv) => pv + cv, 0); // sum all sites
			...
			this.querySelectorAll(".highlightable")[suggestionTier].classList.add(
				"highlight",
			);
			this.querySelector("#modal-pwyw .highlight + div input").setAttribute(
				"checked",
				"checked",
			);

			const val = $("input[type=radio][name=plan]:checked").val();
